...
Component({
  ...
  methods: {
    ...
    _addMonitor: function _addMonitor() {
    ...
    _receiveHis: function _receiveHis(msgList) {
      console.log('hismsg count:', msgList.length);
      that._addToMsgList(msgList, reloadMsg);
    },


    /** 收到即时消息 */
    _receiveMsg: function _receiveMsg(msg) {
    ...
    /** 将收到的消息加入数组 */
    _addToMsgList: function _addToMsgList(msgList, reload) {
    ...
    _precessMsg: function _precessMsg(msg) {
      //将unix时间戳格式化成文本
      ...
      if ('card' == msg.type && 'buyDrugInformation' == msg.bodyContent.command) {
        //获取药卡的最新状态
    ...
    _updateDrugOrderStatus: function _updateDrugOrderStatus(orderId) {
      var url = this._getHost().wmpHost + 'im/getDrugOrderInfo?orderId=' + orderId + '&userToken=' + this.data._request.userToken;
      wx.request({
        url: url,
        data: {},
        method: 'POST',
    ...
    _updateDrugOrderCommandText: function _updateDrugOrderCommandText(orderId) {
      wx.setStorage({
    ...
    /** 显示或隐藏呼叫医生弹出层 */
    _showHideBtn: function _showHideBtn() {
      this._showBtn(!this.data.callBtnsVisible);
    },


    /** 隐藏呼叫医生弹出层 */
    _hideBtn: function _hideBtn() {
      this._showBtn(false);
    ...
    /** 动画修改呼叫医生弹出层的高度 */
    _showBtn: function _showBtn(visible) {
    ...
    /** 显示或隐藏底部的工具栏 */
    _showHideUtils: function _showHideUtils() {
    ...
    /** 修改底栏高度 */
    _showUtils: function _showUtils(visible) {
      this.setData({
    ...
    /** 点击呼叫医生 */
    _callDoctor: function _callDoctor(e) {
      this._hideBtn();
    ...
    /** 切换显示语音输入和文字输入 */
    _changeInputType: function _changeInputType() {
      this._showUtils(false);
    ...
    /** 发送文本消息 */
    _sendTextMsg: function _sendTextMsg(e) {
    ...
    /** 取消录音 */
    _cancelRecord: function _cancelRecord(e) {
    ...
    /**播放语音 */
    _playAudio: function _playAudio(e) {
    ...
    /** 选择图片或拍照 */
    _selectImage: function _selectImage(e) {
      this._showUtils(false);
    ...
    /** 发送消息回调 */
    _sendCallback: function _sendCallback(res) {
      if (res.allDone) {
        wx.hideLoading();
      }
    },
    ...
    /** 点击查看视频咨询总结卡片 */
    _viewSummary: function _viewSummary(e) {
      this._viewEhr({
        viewModule: 'detail',
    ...
    _getCustomStyle: function _getCustomStyle(style) {
      var str = '';
    ...
    _loadMoreHisMsg: function _loadMoreHisMsg() {
      if (!hhim) {
        return;
      }
      reloadMsg = true;
    ...
    _buyMedicine: function _buyMedicine(e) {
