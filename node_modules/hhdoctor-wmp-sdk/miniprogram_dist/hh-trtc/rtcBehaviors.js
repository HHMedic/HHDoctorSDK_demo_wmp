...
module.exports = Behavior({
  ...
  data: {
    ...
    enterChooseImages: false,  //是否进入选图片或拍照
    enterShare: false,   //是否进入分享
  ...
  methods: {
    ...
    _registMessageReceived() {
      hhDoctor.on('messageReceived', function (e) {
        ...
        e.map((item, index) => {
          ...
          if (self.data.preReceivedCommand == obj.command && obj.command != 'waitUserInfo' && obj.command != 'user_certification') {
            return;
          }
    ...
    sendMessage(command, newOrderid, fromId, success, fail) {
      ...
      return new Promise((resolve, reject) => {
        ...
        if (data.orderid) {
          ...
          let toUser = (command == 'busy' || command == 'ACCEPT') && fromId ? fromId : uuid;
    ...
    _workbenchRegist() {
      ...
      self.data.workbenchList['user_certification'] = self.workbenchUserAuth;
    ...
    bindToggleTorch() {
      ...
      //_request.localVideoStatus
      if (-1 == self.data._request.localVideoStatus) return wx.showToast({ title: '不支持打开闪光灯', icon: 'none' })
      if (0 == self.data._request.localVideoStatus) return wx.showToast({ title: '请先打开摄像头', icon: 'none' })
    ...
    _requestCreateFamOrder(famOrderId, deptId) {
      ...
      self.data.createOrderPromise = apis.requestCreateFamOrder(deptId ? deptId : self.data._request.dept, famOrderId, orderType,
        ...
        self.data._request.hospitalId, self.data._request.ext, self.data._request.callType, self.data.waitRoom).then(res => {
          console.log('创建订单====>', res)
          self.data.waitRoom = ''
          if (res.status == 200) {
            self.requestRtcLog('1', 'createFamOrder success', res.data.order.orderid);
            let data = res.data;
            let orderid = data.order.orderid;
            let roomID = orderid.slice(orderid.length - 6, orderid.length)
            let isEnterWaitRoom = false
            if ('undefined' != typeof data.isEnterWaitRoom) isEnterWaitRoom = data.isEnterWaitRoom
            self.requestRtcLog('1', 'createFamOrder roomID:' + roomID, res.data.order.orderid);
            self.requestRtcLog('1', 'createFamOrder setData userSig:' + hhDoctor.getUserSig(), res.data.order.orderid);
            let preCallResponse = data.preCallResponse;
            self.requestRtcLog('1', 'createFamOrder preCallResponse:' + JSON.stringify(preCallResponse), res.data.order.orderid);
            self.setData({
              orderid,
              roomID,
              userID: data.uuid,
              ...
              isEnterWaitRoom,
              realPatientName: data.name,
              realPatientUserToken: data.realPatientUserToken,
              isAuth: data.isAuth,
              callTips: {
              ...
              self.requestRtcLog('1', 'createFamOrder setData finish', self.data.orderid);
              console.error('>>> createFamOrder :', self.data.doctor, self.data.isEnterWaitRoom)
              if (data.doctor && !self.data.isEnterWaitRoom) {
                //进入排队
                ...
                self.triggerEvent('entercall', { order: data.order, doctor: data.doctor })
              } else {
                //被邀请，显示提示
          ...
          self.requestRtcLog('1', 'createFamOrder catch:' + JSON.stringify(err))
    ...
    requestHangUp(reason, isBack) {
      ...
      apis.requestHangUp(params).then(res => {
        ...
        if (res.status == 200) {
          console.error('>>> requestHangUp', res.data, data.isClickHangUp, data.isInvite)
    ...
    _naviBack() {
      ...
      if (!self.isOnlyOnePage()) {
        wx.navigateBack()
        //延时500ms检测状态，如果页面未返回，尝试再次返回
        setTimeout(() => { if (self.data.pageStatus != 'detached' && self.data.isInvite != 1) wx.navigateBack() }, 500)
        return
      }
