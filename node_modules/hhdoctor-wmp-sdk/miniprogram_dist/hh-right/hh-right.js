...
Component({
  ...
  methods: {
    ...
    _getProductInfo() {
      if (this.data._request.pid) this._getProductDetailByPid()
      else this._getMyProductDetail()
    },
    _getMyProductDetail() {
    ...
    _getProductDetailByPid() {
      var url = this._getHost().wmpHost +
        'my/productDetail?userToken=' + this.data._request.userToken +
        '&sdkProductId=' + this.data._request.sdkProductId +
        '&productId=' + this.data._request.pid;
      wx.request({
        url: url,
        data: {},
        method: 'POST',
        success: function (res) {
          if (res && res.data && 200 == res.data.status) {
            //成功
            that.setData({
              product: res && res.data && res.data.data || {}
            })
          }
        }
      })
