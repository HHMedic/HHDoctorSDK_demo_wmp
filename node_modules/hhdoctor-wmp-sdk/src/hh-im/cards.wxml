      ...
      <view class='msg-content-main {{"c"==from?"user":""}}'>
        <view class='msg {{"c"==from?"user":""}} text'>
          <block wx:for="{{textArr}}" wx:key="this">
            <text class="{{'link'==item.type?'text-link':''}}" catchtap="{{'link'==item.type?'_tapLink':''}}"
              data-link="{{item.value}}">{{'link'==item.type?item.name:item.value}}</text>
          </block>
        </view>
      </view>
    </view>
    ...
    <view class='msg-body card'>
      ...
      <view class='msg-content-main'>
        <view class='msg card'>
          ...
          <view class='msg-card-foot' wx:if='{{bodyContent.trans}}' catchtap='_buyMedicine' data-id='{{id}}'
            data-order-id="{{bodyContent.orderId}}" data-medic-record-id='{{body.medicRecordId?body.medicRecordId:""}}'
            data-source='{{bodyContent.source?bodyContent.source:""}}'
            data-patient-uuid='{{body.patientUuid?body.patientUuid:""}}' data-drugid='{{bodyContent.drugOrderId}}'
            data-trans='{{bodyContent.trans}}' data-carturl='{{bodyContent.cartUrl?bodyContent.cartUrl:""}}'>
            <view class='show-more'>
              <view class='show-more-btn {{bodyContent.trans?"":"disabled"}}' style="{{bodyContent.trans?btnStyle:''}}">
...
<template name='netHospitalRx'>
  <view class='msg-row' id='msg{{time}}'>
    ...
    <view class='msg-body card'>
      <view class='msg-name'>{{name}}</view>
      <view class='msg-content-main'>
        <view class='msg card' catchtap='_viewNetHospitalRxDetail' data-rx-id="{{bodyContent.rxId}}"
          data-rx-url="{{bodyContent.rxUrl}}">
...
<!-- 19.fastchannel  重疾绿通 、nurse_home  医护到家 、hao 挂号服务-->
  ...
  <view class='msg-row' id='msg{{time}}'>
    ...
    <view class='msg-body card'>
      ...
      <view class='msg-content-main'>
        ...
        <block>
          <view class='msg card'>
            <view class='msg-card-head'>{{bodyContent.title
              ||(bodyContent.command=='fastchannel'?'住院绿通':bodyContent.command=='hao'?'挂号绿通':'护士上门')}}</view>
            <view class='msg-card-body summary'>
            ...
            <view class='msg-card-foot' catchtap="_tapAppointmentViewUrl" data-id='{{id}}'
              data-is-success="{{body.isSuccess}}" data-real-patient-user-token='{{bodyContent.realPatientUserToken}}'
              data-coop-order-id="{{bodyContent.coopOrderId}}" data-title="{{bodyContent.title}}"
              data-url="{{bodyContent.forwardUrl}}">
              <view class='show-more'>
                <view class='show-more-btn {{body.isSuccess==1?"disabled":""}}' style='color:#fff'>
                  {{body.isSuccess==1?"已预约":"立即预约"}}</view>
              </view>
            </view>
          </view>
        </block>
...
<!-- 20.hao_detail 挂号详情、fastchannel_detail 重疾绿通详情 、nurse_home_detail 医护上门详情-->
<template name='appointmentItemDetail'>
  <view class='msg-row' id='msg{{time}}'>
    ...
    <view class='msg-body card'>
      <view class='msg-name'>{{name}}</view>
      <view class='msg-content-main'>
        <view class='msg card' catchtap='_viewOriginUrl' data-is-success="{{body.isSuccess}}"
          data-real-patient-user-token='{{bodyContent.realPatientUserToken}}'
          data-coop-order-id="{{bodyContent.coopOrderId}}" data-url="{{bodyContent.forwardUrl}}">
          <view class='msg-card-head'>{{bodyContent.title}}</view>
          <view class='msg-card-body summary'>
            <view class='summary-row'>
