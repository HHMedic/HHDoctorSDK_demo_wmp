...
Component({
  ...
  methods: {
    ...
    getRateContentList(){
      ...
      console.log('self.data.evaluateData',self.data.evaluateData)
    ...
    bindChangeDoctor(){
      apis.requestChangeDoctor(self.data.famOrderId).then(res=>{
        if(res.status==200){
          ...
          var pageUrl =  '/pages/room/room'+ '?' + hhDoctor.getPublicParams()+'&dept='+self.data._request.dept+'&uuid='+self.data.uuid;
          wx.redirectTo({
            url: pageUrl
          })
        }
