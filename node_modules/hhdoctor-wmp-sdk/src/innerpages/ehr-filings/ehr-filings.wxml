   ...
   <view class='filing-box' wx:if='{{medicrecords.length}}' wx:for='{{medicrecords}}' wx:key='index' bindtap='bindFilingItem' data-id='{{item.medicRecordId}}' >
      ...
      <view class='filing-content'>
        <view class='filing-text'>{{item.medicRecordSummaryData.diseaseDescn}}</view>
         ...
         wx:if='{{item.medicRecordExamineData.checkPics.length || item.medicRecordExamineData.medicRecordPics.length || item.medicRecordExamineData.dicomPics.length||item.medicRecordExamineData.dicoms.length}}'>
            <list-images name='病例'  
            ...
            <list-images name='DICOM'  
               base-path="../../"
               imgUrls='{{item.medicRecordExamineData.dicoms}}' 
               memberUuid='{{memberUuid}}'
               medicRecordId='{{item.medicRecordId}}'
               wx:if='{{item.medicRecordExamineData.dicoms.length}}' >
            </list-images> 
        </scroll-view>
      </view>
   </view>
