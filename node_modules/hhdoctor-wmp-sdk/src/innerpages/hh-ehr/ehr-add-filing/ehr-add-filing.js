// components/innerpages/hh-ehr/ehr-add-filing/ehr-add-filing.js
const app = getApp();
...
Page({
  ...
  data: {
    ...
    list: [{
        isCheck: false,
        title: '提供影像DICOM',
        texts: ['什么是Dicom资料？', '  Dicom是医疗影像设备（X射线、CT、核磁共振、超声等）检查后的图像保存格式', 'Dicom资料如何上传', '  首先选择【提供影像dicom资料】并保存本次档案，然后在电脑上将影像文件放到文件夹内并压缩成.zip或.rar文件，文件名修改为视频医生账号（手机号）+患者姓名', '方式一', ' 1、电脑打开e.hh-medic.com用本账号登陆', ' 2、点击档案后面+号选择影像压缩文件并上传', ' 3、上传成功后，30分钟内系统会自动关联您的健康档案', '方式二', ' 1、将影像压缩文件通过邮件方式发送到dicom@hh-medic.com邮箱', ' 2、我们会在收到邮件后12小时内关联您的健康档案', 'Dicom获取方式', ' 1、去医院影像科索要光盘（部分医院需要支付费用购买），然后按照上面步骤上传。', ' 2、如果医院不提供光盘，可以拍摄照片，然后从上面病历资料入口上传照片即可。', '（1）可到医院科室用看片灯观看胶片，然后对着胶片拍照，每张照片只拍摄胶片的4个小画面，确保小画面日期可被清晰显示', '（2）可在家中找一个显示器，然后将显示器调整为纯白色桌面，把胶片放到显示器前拍照，同样每张照片只拍摄4个小画面。']
      },
      {
        isCheck: false,
        title: '提供病理切片',
  ...
  bindSubmitSave: function() {
    ...
    if (this.data.isDbClick) {
      this.data.isDbClick = false;
      wx.showLoading({ mask: true })
      apis.requestAddEhr(params).then(res => {
        wx.hideLoading();
        this.data.isDbClick = true;
        if (res.status == 200) {
          wx.navigateBack();
        } else {
          wx.showToast({
