...
Page({
    ...
    bindAddFimily() {
        ...
        if (!this.checkMemberMessage(memberList[0].name, memberList[0].sex, memberList[0].birthday, memberList[0].needUpdateInfo)) {
            this.setData({
                modalMsgData: { source: 'member', content: infoTip + '才可添加成员', confirmText: '立即补充' },
                isShowModal: true,
                memberUuid: memberList[0].uuid
            })
            return;
        }
        ...
        let userNum = this.data.userCnt - (memberList.length - 1) - this.data.deleteCount;
    ...
    navigateAddMember() {
        wx.navigateTo({
            url: `../ehr-addmember/ehr-addmember`
        });
    },
    //编辑家庭成员信息
    bindFillingEdit(e) {
        ...
        wx.navigateTo({
            url: `../ehr-addmember/ehr-addmember?type=3&memberUuid=${uuid}`
        });
    },
    //进入详情页
    bindItemNavigator: function (e) {
        ...
        if (!this.checkMemberMessage(e.currentTarget.dataset.name, e.currentTarget.dataset.sex, e.currentTarget.dataset.birthday)) {
            this.setData({
                modalMsgData: { source: 'member', content: infoTip + '才可查看详情', confirmText: '立即补充' },
                isShowModal: true,
    ...
    bindDelete: function (e) {
        ...
        let self = this;
        if (memberUuid == self.data.patient.uuid) {
            wx.showToast({
                title: '您不能删除自己',
                icon: 'none',
                duration: 1000
