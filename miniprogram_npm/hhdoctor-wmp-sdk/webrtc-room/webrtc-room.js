...
Component({
  ...
  properties: {
    ...
    waitingImg: {
      type: String,
      value: 'https://imgs.hh-medic.com/icon/wmp/img_busy.png'
    },

    // live-player的背景图
    playerBackgroundImg: {
      type: String,
      value: 'https://imgs.hh-medic.com/icon/wmp/bg-default.jpg'
    },
  ...
  pageLifetimes: {
    show: function() {
      let self = this;
      self.getImgSetting(self.data.systeminfo.screenWidth, self.data.systeminfo.screenHeight)
    }
  },
  ...
  methods: {
    getImgSetting(w, h) {
      self = this;
      let refer = 0.56221889;
      let width = Math.ceil(h * refer);
      let left = Math.ceil(Math.abs(w - width) / 2);
      let height = '100%'
      if (w >= 768) {
        width = w;
        left = 0;
        height = Math.ceil(w / refer) + 'px'
      }
      self.setData({
        width: width + 'px',
        left: left + 'px',
    ...
    startPreview: function() {
      ...
      self.data.pusherContext && self.data.pusherContext.startPreview();
    },
    stopPreview: function() {
      if (!self.data.pusherContext) {
        self.data.pusherContext = wx.createLivePusherContext('rtcpusher');
      }
      self.data.pusherContext && self.data.pusherContext.stopPreview();
    },
    /**
     * webrtc-room程序的入口
     */
    ...
    startPlay: function() {
      //如开始播放时未收到医生进房间消息，则设置为自动播放，当医生进房间后自动播放
      if (self.data.members &&
        0 == self.data.members.length) {
        self.setData({
          autoplay: true
        })
      }
    ...
    toggleTorch: function() {
      ...
      self.data.pusherContext && self.data.pusherContext.toggleTorch({});
      ...
      wx.request({
        ...
        success: function(res) {
          ...
          setTimeout(() => {
            ...
            self.data.pusherContext && self.data.pusherContext.start && self.data.pusherContext.start();
    ...
    onPusherJoin: function(res) {
      ...
      res.pushers.forEach(function(val) {
        ...
        if (!hasPlay && emptyIndex != -1) {
          val.loading = false;
          self.requestRtcLog('2', 'onPusherJoin:' + JSON.stringify(res), self.data.orderid)
    ...
    onPush: function(e) {
      ...
      self.postEvent('push', e.detail.code, e.detail.message);
      if (e.detail.code != 1020) {
        console.log('on-push', e.detail.message, e.detail.code)
        self.requestRtcLog('2', `statusCode:${e.detail.code}(${e.detail.message})`, self.data.orderid)
      }
      if (!self.data.pusherContext) {
        self.data.pusherContext = wx.createLivePusherContext('rtcpusher');
      }
      ...
      // console.log('推流事件：', e.detail.message,code);
      var errmessage = '';
      switch (code) {
        case 1002:
          {
        ...
        case -1301:
          {
            ...
            self.postErrorEvent(-1301, '打开摄像头失败,请重启微信再试~');
            break;
          }
        case -1302:
          {
            ...
            self.postErrorEvent(-1302, '打开麦克风失败,请重启微信再试~');
            break;
          }
        case -1303:
          {
            self.postErrorEvent(-1303, '视频编码失败,请重启微信再试~');
            break;
          }
        case -1304:
          {
            self.postErrorEvent(-1304, '音频编码失败,请重启微信再试~');
            break;
          }
        case -1305:
          {
            self.postErrorEvent(-1305, '不支持的视频分辨率');
            break;
          }
        case -1306:
          {
            self.postErrorEvent(-1306, '不支持的音频采样率');
            break;
          }
        ...
        case -1318:
          {
            self.postErrorEvent(-1318, '录音失败,请重启微信');
            break;
          }
        case 5000:
          {
        ...
        case 1020:
          {
            ...
            // self.requestRtcLog('2', '成员列表' + JSON.stringify(e.detail.message), self.data.orderid)
            break;
          }
        ...
        case 3005:
          {
            //self.postErrorEvent(self.data.ERROR_PUSH_DISCONNECT, '网络故障');
            self.postErrorEvent(3005, '网络故障');
            //self.exitRom();
            break;
          }
    ...
    //播放器live-player回调
    onPlay: function(e) {
      self.postEvent('play', e.detail.code, e.detail.message);
      if (e.detail.code != 2005) {
        self.requestRtcLog('3', `statusCode:${e.detail.code}(${e.detail.message})`, self.data.orderid)
      }
      self.data.members.forEach(function(val) {
