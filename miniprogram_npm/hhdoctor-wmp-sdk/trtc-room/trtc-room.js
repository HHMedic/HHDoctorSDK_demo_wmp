...
Component({
  ...
  data: {
    ...
    patient: null,
    menuButton: {},
    patientStatusEnum: PATIENT_STATUS_ENUM
  ...
  methods: {
    ...
    enterRoom(params) {
      return new Promise((resolve, reject) => {
        ...
        self.requestRtcLog('2', 'TRTC-enterRoom', self.data.orderid)
        // 1. 补齐进房参数，校验必要参数是否齐全
        if (params) {
    ...
    exitRoom(isUserExit) {
      ...
      if (this.status.pageLife === 'hide') {
        ...
        self.requestRtcLog('2', TAG_NAME + '小程序最小化时不能调用 exitRoom', self.data.orderid)
      ...
      return new Promise((resolve, reject) => {
        ...
        self.requestRtcLog('2', `TRTC-exitRoom:${isUserExit ? isUserExit : 'sdk'}`, self.data.orderid)
        ...
        this.data.pusher.getPusherContext().stop()
        ...
        this.setData({
          ...
          self.requestRtcLog('2', 'TRTC-exitRoom success', self.data.orderid)
    ...
    publishLocalVideo() {
      ...
      if (this.properties.localVideoStatus < 1) {
        this._setPusherConfig({ enableCamera: false })
        let msg = '用户不开启本地摄像头';
        console.log(TAG_NAME, msg)
    ...
    unpublishLocalVideo() {
      ...
      self.requestRtcLog('2', 'TRTC-unpublshLocalVideo 关闭摄像头', self.data.orderid)
      if (!pusher) pusher = wx.createLivePusherContext()
      setTimeout(() => pusher.stopPreview(), 100)
      pusher.stopPreview()
      return this._setPusherConfig({ enableCamera: false, autopush: false })
    },
    getHandlePush() {
      if (!pusher) pusher = wx.createLivePusherContext()
    ...
    publishLocalAudio() {
      ...
      self.requestRtcLog('2', 'TRTC-publishLocalAudio 开启麦克风', self.data.orderid)
    ...
    unpublishLocalAudio() {
      ...
      self.requestRtcLog('2', 'TRTC-unpublshLocalAudio 关闭麦克风', self.data.orderid)
    ...
    toggleTorch(obj) {
      ...
      this.data.pusher.getPusherContext().toggleTorch({
        success(res) {
          obj && obj.success()
        }
      });
    ...
    _getPushUrl(rtcConfig) {
      ...
      self.requestRtcLog('2', '_getPushUrl', self.data.orderid)
      ...
      if (ENV.IS_TRTC) {
        ...
        return new Promise((resolve, reject) => {
          ...
          setTimeout(() => {
            ...
            self.requestRtcLog('2', 'getPushUrl result:' + pushUrl, self.data.orderid)
    ...
    _requestSigServer(rtcConfig) {
      ...
      self.requestRtcLog('2', '_requestSigServer', self.data.orderid)
      ...
      self.requestRtcLog('2', '_requestSigServer:' + url, self.data.orderid)
      ...
      return new Promise((resolve, reject) => {
        wx.request({
          ...
          success: (res) => {
            ...
            self.requestRtcLog('2', '_requestSigServer success:' + JSON.stringify(res), self.data.orderid)
          ...
          fail: (res) => {
            ...
            self.requestRtcLog('2', '_requestSigServer fail' + JSON.stringify(res), self.data.orderid)
    ...
    _bindEvent() {
      ...
      this.userController.on(EVENT.REMOTE_VIDEO_ADD, (event) => {
        self.requestRtcLog('3', '远端视频可用:' + JSON.stringify(event), self.data.orderid)
      ...
      this.userController.on(EVENT.REMOTE_AUDIO_ADD, (event) => {
        self.requestRtcLog('3', '远端音频可用:' + JSON.stringify(event), self.data.orderid)
    ...
    _pusherStateChangeHandler(event) {
      ...
      self.requestRtcLog('2', 'pusherStateChange:' + code + ' messge:' + message, self.data.orderid)
      ...
      switch (code) {
        ...
        case -1301:
          ...
          this._emitter.emit(EVENT.ERROR, { code, msg: '打开摄像头失败' })
          break
        case -1302:
          console.error(TAG_NAME, '打开麦克风失败: ', code)
          this._emitter.emit(EVENT.ERROR, { code, msg: '打开麦克风失败' })
          break
        case -1303:
          console.error(TAG_NAME, '视频编码失败: ', code)
          this._emitter.emit(EVENT.ERROR, { code, msg: '视频编码失败' })
          break
        case -1304:
          console.error(TAG_NAME, '音频编码失败: ', code)
          this._emitter.emit(EVENT.ERROR, { code, msg: '音频编码失败' })
          break
        case -1307:
        ...
        case -3301:
          ...
          this._emitter.emit(EVENT.ERROR, { code, msg: '网络故障，进房失败' })
          break
        case -100018:
          console.error(TAG_NAME, '进房失败: userSig 校验失败，请检查 userSig 是否填写正确', code, message)
          this._emitter.emit(EVENT.ERROR, { code, msg: '进房失败' })
          break
          // case 4999:
          //   console.error(TAG_NAME, '4999', code, message)
          //   this._emitter.emit(EVENT.ERROR, { code, msg:'4999' })
          break
        case 5000:
        ...
        case 5001:
          ...
          if (this.status.isPush) {
            this._emitter.emit(EVENT.ERROR, { code, msg: '小程序悬浮窗被关闭' })
            this.exitRoom();
          }
          break
        case 1021:
    ...
    _pusherNetStatusHandler(event) {
      ...
      if (this.status.pageLife == 'hide') return;
    ...
    _tapVideoControl() {
      this.setData({ localVideoStatus: 1 - this.properties.localVideoStatus })
      if (this.properties.localVideoStatus > 0) {
        this.publishLocalVideo()
      } else {
        this.unpublishLocalVideo()
      }
      this._emitter.emit('CUSTOMER_EVENT_OPEN_CLOSE_LOCAL_VIDEO', { localVideoStatus: this.properties.localVideoStatus })
    },
    ...
    _playerStateChange(event) {
      ...
      console.log('playStateChange:', self.data.orderid, event)
    ...
    _playerNetStatus(event) {
      console.log('playNetStatus:', self.data.orderid, event.detail.info)
      ...
      let info = event.detail.info;
      let fps = Math.ceil(info.videoFPS)
      self.requestRtcLog('3', 'netinfo:' + `{"vbr":${info.videoBitrate},"abr":${info.audioBitrate},"fps":${fps},"spd":${info.netSpeed},"vw":${info.videoWidth},"vh":${info.videoHeight}}`, self.data.orderid);
    },
